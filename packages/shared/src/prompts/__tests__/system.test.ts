...
describe('system prompt guidance', () => {
  ...
  it('treats OfficeCLI as a native capability rather than a skill', () => {
    ...
    expect(prompt).not.toContain('doc-diff old.docx new.docx')
    ...
    expect(prompt).toContain('office_document_guide')
    expect(prompt).toContain('office_document_preview')
    expect(prompt).toContain('office_document_finalize')
    expect(prompt).toContain('Do not load a user Skill named officecli')
    expect(prompt).toContain('official skill bootstrap')
    expect(prompt).toContain('internal runtime capabilities, not user-installable Skills')
    expect(prompt).toContain('Office document workflow')
    expect(prompt).toContain('Pass native tokens as argv')
    expect(prompt).toContain('Existing outputs require explicit --force')
    expect(prompt).toContain('Batch is atomic unless --best-effort')
    expect(prompt).toContain('Only office_document_preview.start may open or focus')
    expect(prompt).toContain('deliveryReady means the official skill Delivery Gate passed')
    expect(prompt).toContain('not Microsoft Office human visual approval')
