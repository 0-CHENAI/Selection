...
describe('system prompt guidance', () => {
  ...
  it('treats OfficeCLI as a bundled skill plus Bash CLI', () => {
    ...
    expect(prompt).toContain('Prefer typed OfficeCLI tools')
    expect(prompt).toContain('python-docx')
    expect(prompt).toContain('do not curl-install')
    expect(prompt).toContain('Read the automatically gated `officecli` skill first')
    expect(prompt).toContain('load only the format the user needs')
