...
describe('system prompt guidance', () => {
  ...
  it('treats OfficeCLI as a native capability rather than a skill', () => {
    ...
    expect(prompt).toContain('batchCommandsFile')
    expect(prompt).toContain('do not retry the same payload')
    expect(prompt).toContain('multiple in-limit batches')
    expect(prompt).toContain('view outline')
    expect(prompt).toContain('at most one validate')
    expect(prompt).toContain('dump only prepares a replayable batch')
    expect(prompt).toContain('field_not_evaluated')
    expect(prompt).toContain('Word + Windows')
    expect(prompt).toContain('help docx heading')
