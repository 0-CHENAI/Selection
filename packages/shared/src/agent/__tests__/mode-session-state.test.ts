...
describe('mode transition session_state context', () => {
  it('includes explicit transition metadata after a mode change', () => {
    ...
    expect(diagnostics.transitionDisplay).toBe('Read-only Explore -> Full Takeover');
    ...
    expect(stateBlock).toContain('modeTransition: Read-only Explore -> Full Takeover');
    ...
    expect(stateBlock).toContain('modeChangeSummary: Last mode change by user at 2026-03-02T08:00:00.000Z (Read-only Explore -> Full Takeover, modeVersion=2)');
