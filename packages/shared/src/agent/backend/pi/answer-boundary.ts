...
export const MARKER_ANSWER_PROMPT = `Use marker-v1 for this reply. Before the marker, write only brief progress notes when actually using tools, never an answer draft. If no tools are needed, start immediately with the marker. After all tools finish, emit <<<FINAL_ANSWER>>> alone on a line, then only the standalone user-facing answer. Do not include progress recaps or introductory delivery announcements after the marker. Do not call tools after the marker. Short answers and clarification questions also require the marker. Never put the marker in a code block.`
