...
import { chmodSync, copyFileSync, existsSync, linkSync, mkdirSync, mkdtempSync, readFileSync, rmSync, writeFileSync } from 'fs'
...
describe('OfficeCLI attribution intent', () => {
  it('distinguishes research and troubleshooting from explicit, scoped disclosure requests', () => {
    for (const message of [
      ...
      '调研 OfficeCLI 的归因机制并输出 Word 报告',
      '请在报告中添加一节 OfficeCLI 归因机制分析',
      '分析 OfficeCLI attribution 的设计并生成文档',
      'Add a section about OfficeCLI attribution',
      'Include OfficeCLI attribution analysis in the report',
      'Include an analysis of OfficeCLI attribution in the report',
      'Explain how to credit OfficeCLI in a technical guide',
      '请介绍如何署名 OfficeCLI',
      '请添加 OfficeCLI 署名机制章节',
      '解释 OfficeCLI 署名规则，写成 Word',
    ...
    expect(getOfficecliAttributionPolicy('请将文档作者设为 OfficeCLI')).toBe('allow-metadata')
    expect(getOfficecliAttributionPolicy('请在属性中将作者设为 OfficeCLI')).toBe('allow-metadata')
    expect(getOfficecliAttributionPolicy('请在作者栏注明 OfficeCLI')).toBe('allow-metadata')
    expect(getOfficecliAttributionPolicy('请在页脚添加 OfficeCLI 署名')).toBe('allow-visible')
    expect(getOfficecliAttributionPolicy('请署名 OfficeCLI')).toBe('allow-visible')
    expect(getOfficecliAttributionPolicy('请在文档中署名 OfficeCLI')).toBe('allow-visible')
    expect(getOfficecliAttributionPolicy('请分析 OfficeCLI 的设计，并在文档中署名 OfficeCLI')).toBe('allow-visible')
    expect(getOfficecliAttributionPolicy('请在报告中加入 OfficeCLI 署名，并添加一节性能分析')).toBe('allow-visible')
    expect(getOfficecliAttributionPolicy('Research OfficeCLI and include an OfficeCLI credit in the footer.')).toBe('allow-visible')
    expect(getOfficecliAttributionPolicy('Add an analysis section, and credit OfficeCLI in the footer.')).toBe('allow-visible')
    expect(getOfficecliAttributionPolicy('请在页脚和文档 creator 字段中保留 Generated by OfficeCLI attribution')).toBe('allow-all')
...
describe('OfficeCLI POSIX wrapper fail-closed preflight', () => {
  it('does not start a mutation when the trusted sanitizer runtime is unavailable', () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'officecli-wrapper-preflight-'))
    try {
      ...
      for (const args of invocations) {
        ...
        expect(result.stderr.toString()).toContain('failed closed before mutation')
      }
  ...
  it('does not start a mutation when the attribution sanitizer is unavailable', () => {
    if (process.platform === 'win32') return
    const root = mkdtempSync(join(tmpdir(), 'officecli-wrapper-sanitizer-preflight-'))
    try {
      const binDir = join(root, 'resources', 'bin')
      const platformDir = join(binDir, `${process.platform}-${process.arch}`)
      mkdirSync(platformDir, { recursive: true })
      ...
      const trustedBun = join(root, 'vendor', 'bun', 'bun')
      mkdirSync(join(root, 'vendor', 'bun'), { recursive: true })
      writeFileSync(trustedBun, '#!/bin/sh\nexit 0\n')
      chmodSync(trustedBun, 0o755)
