...
describe('parseTasklistImageName', () => {
  it('extracts the image name from a CSV data row', () => {
    expect(parseTasklistImageName('"Selection.exe","1234","Console","1","150,000 K"')).toBe('Selection.exe')
  })
...
describe('lockHolderMatchesLock (#978)', () => {
  ...
  it('matches on identical execName', () => {
    expect(lockHolderMatchesLock(lockWith('Selection.exe'), 'Selection.exe', null)).toBe(true)
  })
  ...
  it('rejects a different executable even when its name contains "craft"', () => {
    ...
    expect(lockHolderMatchesLock(lockWith('Selection.exe'), 'minecraft-launcher', null)).toBe(false)
  })

  it('fails open when the live exec name cannot be determined', () => {
    expect(lockHolderMatchesLock(lockWith('Selection.exe'), null, null)).toBe(false)
  })
  ...
  it('ignores the command line when execName is recorded', () => {
    ...
    expect(lockHolderMatchesLock(lockWith('Selection.exe'), 'java', '/usr/bin/java -jar minecraft.jar')).toBe(false)
  })

  describe('legacy locks without execName', () => {
    it('falls back to the app-name substring heuristic on the command line', () => {
      expect(lockHolderMatchesLock(lockWith(undefined), null, '/Applications/Selection.app/Contents/MacOS/Selection')).toBe(true)
      // Pre-rename process paths still count as ours
