...
import { buildAgentRuntimeConfigUpdate, SessionManager, createManagedSession } from './SessionManager.ts'
...
describe('refreshConnectionRuntime', () => {
  ...
  it('records customModels with the per-model supportsImages flag in the IPC payload', () => {
    // Use an explicit context rather than resolving a user-level connection.
    // CI has no ~/.selection connection catalog, while a developer machine
    // may happen to contain the old slug used by this regression test.
    const payload = buildAgentRuntimeConfigUpdate({
      connection: {
        slug: 'shape-check',
        name: 'Shape check',
        ...
        models: [
          { id: 'vision', supportsImages: true } as never,
          { id: 'text', supportsImages: false } as never,
        ],
      },
      provider: 'pi',
      authType: 'api_key',
