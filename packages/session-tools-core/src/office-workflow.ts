...
- Standard DOCX/XLSX/PPTX work (title plus body/cells/shapes plus one table or sheet) must not call status or help. Use this sequence only: office_document_edit create → office_document_edit batch on envelope.documentPath → office_document_inspect argv: ['view', documentPath, 'outline'] → office_document_preview.render → office_document_finalize.
...
- For a complex Word, Excel, PowerPoint, form, paper, dashboard, financial-model, pitch, or Morph task, progressively load only the relevant internal guide topic with office_document_guide before editing. Standard reports do not need a guide catalog crawl.
- Use office_document_inspect for view/get/query/validate/dump/raw. dump and view html write session data/office artifacts for Read. Use status/help only when that payload is not already in this session. Successful identical reads are revision-cached; change the request after an error instead of repeating it.
...
- After a batch that is not the standard five-step recipe, run view issues plus view html (or office_document_preview.render).
