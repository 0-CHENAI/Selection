...
export function DocumentFormattedMarkdownOverlay({
  ...
  const hideDuplicateBody = overlayBodyDuplicatesError(content, error)
  // Clipboard should match what users see (rendered text), not raw markdown source
  const plainCopyContent = useMemo(
    () => markdownToPlainText(hideDuplicateBody ? (error || content) : content),
    [content, error, hideDuplicateBody],
  )
