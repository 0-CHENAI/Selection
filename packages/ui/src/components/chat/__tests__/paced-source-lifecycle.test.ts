...
const resumed=mount([text,true,undefined,'burst']);
assert.equal(resumed.result.text,text,'returning to a live answer must not replay the received prefix');
update(resumed,[text+'追加文字'.repeat(100),true,undefined,'burst']);
assert.equal(resumed.result.text,text,'new deltas must still be queued after returning to a live answer');
step(resumed);assert.ok(resumed.result.text.length>text.length&&resumed.result.text.length<text.length+400);
unmount(resumed);
...
const switched=mount([text,true,undefined,'identity-one']);
for(let i=0;i<20;i++)step(switched);
update(switched,[text,true,undefined,'identity-two']);
assert.equal(switched.result.text,'','a new answer identity starts a new presentation queue even with identical text');
step(switched);assert.ok(switched.result.text.length>0&&switched.result.text.length<100);
unmount(switched);
