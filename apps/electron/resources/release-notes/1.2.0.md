...
- **Swarm and Conductor orchestration technical preview** — Sessions can opt into managed parallel workers or structured DAG execution with explicit lifecycle, budget, recovery, and quality gates. The workbench exposes topology and run state while preserving legacy task behavior unless the preview is enabled. (#177, #186)

## Improvements

- **Skill choices are easier to scan** — Slash-menu skill rows now place the title and truncated description on separate lines, show the full description on hover, use a bare line icon, and keep custom raster icons free of rectangular chrome. (#210, `8d521a01`)
...
- **Swarm final replies include every worker's findings** — Worker results are passed to the coordinator as explicit data packets with stable references, and the final answer must provide a concrete section for each worker followed by a cross-result synthesis. Metadata-only session references and phrase-based aggregation shortcuts no longer pass the delivery gate. (#249, `53e9a775`)
