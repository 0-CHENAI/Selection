...
- **Word `create` seeds Heading styles** — the PATH `officecli` wrapper writes Heading1–3 with `outlineLvl` (plus Title / TOCHeading) so Word can compile a TOC. Heading styles that exist without `outlineLvl` are repaired with `set`, not re-added. Agents must `load_skill word` first and treat `style not found` as a stop.
