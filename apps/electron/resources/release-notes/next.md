...
- **流式正文持续渐显** — 活动段落和代码块只对新增行进行短过渡，旧内容不重复淡入；修复普通输入关闭动画、视口外段落丢失渐显和持续输出时滚动跟随滞后的问题。选择正文立即显示完整内容，历史消息不重播，完成后快速收口。（#328，跟进 #279）
...
- **模型选择器的「模型」提示不再滞留在按钮上方**：桌面输入栏关闭模型下拉框时（点击别处、选中模型、按 Esc、或关闭思考等级子菜单后关闭）不再把焦点交还给触发器，因此提示不会残留、输入框保持可输入，悬停与键盘聚焦时的提示行为不变（#332）。
...
- **聊天界面布局** — 搜索入口移至会话列表标题栏右侧，顶部“列表／新建编排”居中；聊天标题按面板居中，空会话品牌标识调整为原尺寸的 75%。
...
- **编排实例与运行历史** — 主会话保留已停止、失败及完成的运行记录，可切换历史运行并逐一查看并行实例与旧尝试；最近失败运行支持明确重试，停止后迟到创建的子会话不再继续执行。（#297、#304、#305）
