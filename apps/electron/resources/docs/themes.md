...
This guide explains how to customize the visual theme of Selection.

## Overview

Selection uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.selection/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.selection/theme.json` - Override specific colors (app-level)
...
```
~/.selection/workspaces/{id}/config.json
```
...
Create `~/.selection/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.selection/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.selection/themes/{name}.json`
...
1. Create `~/.selection/theme.json` for overrides or `~/.selection/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.selection/theme.json` for overrides, `~/.selection/themes/` for presets)
