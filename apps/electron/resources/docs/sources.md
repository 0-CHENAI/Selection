...
This guide explains how to configure sources (MCP servers, APIs, local filesystems) in Selection.
...
- `~/.selection/workspaces/{workspaceId}/sources/{sourceSlug}/`
...
`~/.selection/provider-domains.json`
...
1. Create the source folder:
   ```bash
   mkdir -p ~/.selection/workspaces/{ws}/sources/my-source
   ```
