...
````
```pdf-preview
{"src":"/absolute/path/to/file.pdf","title":"Result"}
```
````

For tabs, replace `src` with `"items":[{"src":"/absolute/path/to/first.pdf","label":"First"},{"src":"/absolute/path/to/second.pdf","label":"Second"}]`. Each path must exist. Use permission-allowed locations; Explore writes are restricted to the session's plans/data folders.
