...
export function TaskYamlImport({ workspaceId, onClose, onCreated, target }: Pick<TaskEditorProps, 'workspaceId' | 'onClose' | 'onCreated' | 'target'>) {
  ...
  async function loadFile(file?: File) {
    ...
    try {
      ...
      if (mounted.current && read === fileRead.current) setErrors([String(error)])
    } finally {
  ...
  async function importTask() {
    ...
    try {
      ...
      if (!result.validation.valid) {
        setErrors(result.validation.errors.map(error => `${error.path}: ${error.message}`))
        return
      }
      ...
      if (mounted.current) setErrors([error instanceof Error && error.message === 'size'
        ? t('tasks.yamlImportFileError')
        : error instanceof Error && error.message === 'version'
          ? t('tasks.yamlImportVersionError')
  ...
  return (
    <section className="flex h-full min-h-0 flex-col gap-4 p-6" aria-label={t('tasks.yamlImportTitle')}>
      <h2 className="text-lg font-semibold">{t('tasks.yamlImportTitle')}</h2>
      <p className="text-sm text-muted-foreground">{t('tasks.yamlImportHint')}</p>
      ...
      <textarea className="min-h-40 flex-1 resize-none rounded-md border bg-background p-3 font-mono text-sm"
        aria-label={t('tasks.tabYaml')} spellCheck={false} disabled={busy} value={yaml}
        onChange={event => { setYaml(event.target.value); setErrors([]) }} />
      {errors.length > 0 && <ul role="alert" className="max-h-40 overflow-auto text-sm text-destructive">
