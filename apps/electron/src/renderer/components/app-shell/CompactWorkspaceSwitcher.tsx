 ...
 *
 * Local workspaces only (remote Selection server connection UI removed).
 */
  ...
  const handleRemoveWorkspace = useCallback(async (workspace: Workspace) => {
    ...
    try {
      ...
      if (removed) {
        ...
        toast.error(t('toast.failedToRemoveWorkspace', { name: workspace.name }))
      }
    } catch {
      toast.error(t('toast.failedToRemoveWorkspace', { name: workspace.name }))
    }
        ...
        <DrawerContent>
          ...
          <div className="px-2 pb-2 flex flex-col gap-0.5 max-h-[60vh] overflow-y-auto">
            ...
            <button
              ...
              className="mt-1 flex items-center gap-3 px-3 py-3 rounded-[10px] text-sm text-foreground/70 hover:bg-foreground/5 transition-colors"
            >
