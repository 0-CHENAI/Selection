 ...
 */
import { getAuthState, getSetupNeeds, prepareMcpOAuth } from '@craft-agent/shared/auth'
...
export function registerOnboardingHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  const claudeOAuthUnsupported = {
    success: false as const,
    error: 'Claude Max OAuth is no longer supported. Use ChatGPT, Copilot, or an OpenAI Compatible connection.',
  }

  server.handle(RPC_CHANNELS.onboarding.START_CLAUDE_OAUTH, async () => claudeOAuthUnsupported)

  server.handle(RPC_CHANNELS.onboarding.EXCHANGE_CLAUDE_CODE, async () => claudeOAuthUnsupported)

  server.handle(RPC_CHANNELS.onboarding.HAS_CLAUDE_OAUTH_STATE, async () => false)

  server.handle(RPC_CHANNELS.onboarding.CLEAR_CLAUDE_OAUTH_STATE, async () => ({ success: true }))
