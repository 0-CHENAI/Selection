...
describe('BrowserPaneManager', () => {
  ...
  it('retries toolbar load and recovers', async () => {
    ...
    const toolbarView = createdBrowserViews[0]
    const fileAttempts = toolbarView.webContents.loadFile.mock.calls.length
    const toolbarUrlAttempts = toolbarView.webContents.loadURL.mock.calls
      .filter((args: [string]) => args[0]?.includes('browser-toolbar.html')).length
    const totalAttempts = fileAttempts + toolbarUrlAttempts

    expect(totalAttempts).toBe(3)
    expect(toolbarView.webContents.loadURL).not.toHaveBeenCalledWith(expect.stringContaining('data:text/html'))
  })
  ...
  it('loads toolbar fallback page after retry exhaustion', async () => {
    ...
    expect(toolbarView.webContents.loadURL).toHaveBeenCalledWith(expect.stringContaining('data:text/html'))
  })
