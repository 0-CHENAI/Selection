...
const TASK_DIR = join(ROOT, 'tasks', 'legacy-v2-pure')
const TASK = join(TASK_DIR, 'task.yaml')
const ORIG = `${TASK}.orig`
const HISTORY = join(TASK_DIR, '.history')
const WB = join(ROOT, 'thought-workbenches', 'legacyv2pure', 'document.json')
const OUT = new URL('./thought-workbench-v2-cache-pure-ui.json', import.meta.url)
const PNG = new URL('./thought-workbench-v2-cache-pure-ui.png', import.meta.url)
const V2 = `schema_version: 2
id: legacy-v2-pure
title: Legacy V2 cache-pure
goal: review a note
nodes:
  - id: a
    prompt: hello
...
function seedFixture() {
  ...
  mkdirSync(dirname(WB), { recursive: true })
  writeFileSync(WB, `${JSON.stringify({
    schemaVersion: 1,
    id: 'legacyv2pure',
    revision: 1,
    title: 'Legacy V2 cache-pure',
    ...
    taskSlug: 'legacy-v2-pure',
    taskEtag: createHash('sha256').update(V2, 'utf8').digest('hex'),
    executionYaml: V2,
    nodes: [],
    edges: [],
  })}\n`)
...
async function main() {
  ...
  const switched = await evaluate(session, `(() => {
    const select = [...document.querySelectorAll('select')].find((el) => [...el.options].some((option) => option.value === 'legacyv2pure'))
    if (!select) return { ok: false, options: [...document.querySelectorAll('select option')].map((option) => option.value) }
    Object.getOwnPropertyDescriptor(window.HTMLSelectElement.prototype, 'value').set.call(select, 'legacyv2pure')
  ...
  const dialogAfterSave = await evaluate(session, `({
    ...
    cachePure: document.body.innerText.includes('cache: pure on a becomes run-pure'),
  ...
  const report = {
    ...
    afterConfirm: {
      ...
      runPure: /cache:\s*run-pure/.test(saved),
      stillPure: /cache:\s*pure/.test(saved),
      history: existsSync(HISTORY) ? readdirSync(HISTORY) : [],
    },
  }
  ...
  session.close()
  if (!dialogAfterSave.title || !dialogAfterSave.cachePure || !afterCancel.unchanged || !report.afterConfirm.isV3 || !report.afterConfirm.runPure || report.afterConfirm.stillPure || report.afterConfirm.history.length === 0) {
    process.exitCode = 1
  }
}
