...
Terminal client for Selection server. Connects over WebSocket (`ws://` or `wss://`) to a running headless server.
...
- For all other commands: a running Selection headless server with URL and token
