...
DOWNLOAD_DIR="$HOME/.selection/downloads"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    platform="linux-${arch}"
    APP_NAME="Selection-x64.AppImage"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    if pgrep -x "Selection" >/dev/null 2>&1; then
        info "Quitting Selection..."
        ...
        if pgrep -x "Selection" >/dev/null 2>&1; then
            ...
            pkill -9 -x "Selection" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  Selection has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'Selection'${NC}"
    echo ""
    ...
    # New paths
    APP_DIR="$HOME/.selection/app"
    WRAPPER_PATH="$INSTALL_DIR/craft-agents"
    APPIMAGE_INSTALL_PATH="$APP_DIR/Selection-x64.AppImage"

    # Kill the app if it's running
    if pgrep -f "Craft-Agent.*AppImage" >/dev/null 2>&1; then
        info "Stopping Selection..."
...
#!/bin/bash
# Selection launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.selection/app/Selection-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/@craft-agent"
ELECTRON_CACHE_ALT="$HOME/.cache/@craft-agent"

# Verify AppImage exists
if [ ! -f "$APPIMAGE_PATH" ]; then
    echo "Error: Selection not found at $APPIMAGE_PATH"
...
WRAPPER_EOF
    ...
    OLD_APPIMAGE="$INSTALL_DIR/Selection-x64.AppImage"
