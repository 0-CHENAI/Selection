...
## How it Works (Video)
To understand what Selection does and how it works watch this video.
...
Selection is open source under the Apache 2.0 license - so you are free to remix, change anything. And that's actually possible. We ourselves are building Selection with Selection only - no code editors - so really, any customisation is just a prompt away.

We built Selection because we wanted a better, more opinionated (and preferably non-CLI way) of working with the most powerful agents in the world. We'll continue to improve it, based on our experiences and intuition.
...
Selection connects to anything. We have it hooked up to a direct Postgres DB behind a jumpbox. Skills + Sources = magic.
...
Remote Selection server / thin-client connection has been removed from the personal desktop app. SaaS and personal terminal will be developed separately.

## CLI Client

A terminal client that connects to a running Selection server over WebSocket (`ws://` or `wss://`). Use it for scripting, CI/CD pipelines, server validation, or when you prefer the command line.
...
4. Name: e.g., "Selection Desktop"
5. Click **Create**
6. Note the **Client ID** and **Client Secret**

#### 5. Configure in Selection
...
Selection supports multiple ways to connect to LLM providers:
...
Configuration is stored at `~/.selection/`:
...
Or configure manually in `~/.selection/workspaces/{id}/automations.json`:
...
**Windows (PowerShell):**
```powershell
& "$env:LOCALAPPDATA\Programs\@craft-agentelectron\Selection.exe" -- --debug
```
...
"Craft" and "Selection" are trademarks of Craft Docs Ltd. See [TRADEMARK.md](TRADEMARK.md) for usage guidelines.
